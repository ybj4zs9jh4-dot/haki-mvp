...
// ── CATALOGUE 33 THÉMATIQUES / 9 BLOCS ───────────────────────────
const BLOCS = [
  {
...
const LIEUX = [
  ...
  { id:"hors",       label:"Hors entreprise",          emoji:"🏨", desc:"Salle louée et organisée par Haki" },
  { id:"haki",       label:"Site Haki — Abidjan",      emoji:"📍", desc:"Nos locaux à Abidjan" },
];
const PARTICIPANTS = [
  { id:"p10",  label:"Moins de 10",  emoji:"👥", min:1,  max:9,  coef:1.0 },
  { id:"p30",  label:"10 à 30",      emoji:"👥", min:10, max:30, coef:1.3 },
  { id:"p60",  label:"30 à 60",      emoji:"👥", min:31, max:60, coef:1.6 },
  { id:"p60p", label:"Plus de 60",   emoji:"👥", min:61, max:999, coef:2.0 },
];
const INTERVENANTS = [
  { id:"haki",   label:"Formateur Haki",                emoji:"🎓", desc:"Expert DEI Haki certifié", coef:1.0 },
  { id:"expert", label:"Formateur Haki + Expert invité", emoji:"🎓+⭐", desc:"Juriste CI, médecin travail, RHEEG-CI, expert VIH…", coef:1.5 },
  { id:"seul",   label:"Expert invité coordonné Haki",  emoji:"⭐", desc:"Spécialiste externe, logistique assurée par Haki", coef:1.3 },
];
const DUREES = [
  { id:"demi",  label:"Demi-journée",      emoji:"🕐", heures:"3h",    coef:1.0 },
  { id:"jour",  label:"Journée complète",  emoji:"🕗", heures:"7h",    coef:1.8 },
  { id:"multi", label:"Multi-sessions",    emoji:"📅", heures:"2-5j",  coef:3.5 },
];
const LANGUES = [
  { id:"fr",  label:"Français",              emoji:"🇫🇷" },
  { id:"bil", label:"Bilingue FR / EN",      emoji:"🇫🇷🇬🇧", coef:1.2 },
];
const CERTIFICATIONS = [
  { id:"none",   label:"Sans attestation",               emoji:"—",  coef:1.0 },
  { id:"attest", label:"Attestation de participation",    emoji:"📜", coef:1.05 },
  { id:"certif", label:"Certification Haki DEI Manager",  emoji:"🏅", coef:1.15 },
];
const LIVRABLES = [
  { id:"none",   label:"Sans livrable",                          emoji:"—",   coef:1.0 },
  { id:"rapport",label:"Rapport de formation + évaluation",      emoji:"📊",  coef:1.1 },
  { id:"plan",   label:"Rapport + Plan d'action DEI personnalisé",emoji:"🎯", coef:1.2 },
];

// ── TARIF BASE (FCFA) ────────────────────────────────────────────
const TARIF_BASE = 150000; // demi-journée, <10 participants, formateur Haki, FR, sans options
...
function calculerDevis(config: any): number {
  ...
  const coef = (p?.coef??1) * (i?.coef??1) * (d?.coef??1) * ((l as any)?.coef??1) * (c?.coef??1) * (lv?.coef??1) * lieu;
...
export default function FormationPage() {
  ...
  const [themesSelectionnes, setThemesSelectionnes] = useState<string[]>([]);
  ...
  const nbThemes = themesSelectionnes.length;

  function toggleTheme(id: string) {
    setThemesSelectionnes(prev =>
  ...
  function soumettre() {
    if (nbThemes === 0) { setErreur("Sélectionnez au moins une thématique."); return; }
    setDemandeEnvoyee(true);
  }

  const css = `
    * { box-sizing: border-box; }
    .form-grid { display: grid; grid-template-columns: 1fr 400px; gap: 20px; align-items: start; }
    .opt-grid-2 { display: grid; grid-template-columns: 1fr 1fr; gap: 8px; }
    .opt-grid-3 { display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 8px; }
      ...
      <div style={{ maxWidth:1200, margin:"0 auto", padding:"28px 20px" }}>
        ...
        <div style={{ marginBottom:24 }}>
          ...
          <h1 style={{ fontSize:22, fontWeight:700, color:"#1A237E", margin:"0 0 6px" }}>
            Formations DEI sur mesure pour vos encadrants
          </h1>
        ...
        {nbThemes > 0 && (
          <div style={{ background:"#1A237E", borderRadius:10, padding:"10px 18px", marginBottom:16, display:"flex", alignItems:"center", justifyContent:"space-between" }}>
            <span style={{ fontSize:13, color:"#fff", fontWeight:500 }}>
              ✓ {nbThemes} thématique{nbThemes>1?"s":""} sélectionnée{nbThemes>1?"s":""}
            </span>
            <button className="haki-btn" onClick={() => setThemesSelectionnes([])}
              style={{ padding:"4px 10px", background:"rgba(255,255,255,0.15)", color:"#fff", fontSize:11 }}>
              Tout désélectionner
            </button>
          </div>
        )}
        ...
        <div className="form-grid">
          ...
          {/* ══ GAUCHE — Catalogue ══ */}
          <div>
            <div style={{ fontSize:12, fontWeight:700, color:"#424242", marginBottom:12, letterSpacing:".05em" }}>
              ÉTAPE 1 — Choisissez vos thématiques
            </div>
                ...
                <div key={bloc.id} style={{ background:"#fff", borderRadius:12, overflow:"hidden", boxShadow:"0 1px 3px rgba(0,0,0,0.06)" }}>
                  ...
                  <button className="haki-btn"
                    ...
                    style={{
                      ...
                      background: blocOuvert===bloc.id ? bloc.bg : "#fff",
                    ...
                    <div style={{ flex:1 }}>
                      ...
                      <div style={{ fontSize:10, color:"#9E9E9E", marginTop:1 }}>
                        ...
                        {bloc.themes.filter(t => themesSelectionnes.includes(t.id)).length > 0 && (
                          <span style={{ color:bloc.color, fontWeight:600 }}>
                            {bloc.themes.filter(t => themesSelectionnes.includes(t.id)).length} sélectionnée{bloc.themes.filter(t => themesSelectionnes.includes(t.id)).length>1?"s":""}
                          </span>
                        )}
                      </div>
                    </div>
                  ...
                  {blocOuvert === bloc.id && (
                    <div style={{ padding:"8px 14px 14px", borderTop:`1px solid ${bloc.color}20` }}>
                      {bloc.themes.map(theme => {
                        const sel = themesSelectionnes.includes(theme.id);
                        ...
                        return (
                          <div key={theme.id}
                            ...
                            style={{
                              ...
                              background: sel ? bloc.bg : "#F8F9FA",
                              border:`1.5px solid ${sel ? bloc.color : "transparent"}`,
                              transition:"all .1s",
                            }}>
                            <div style={{
                              width:18, height:18, borderRadius:4, flexShrink:0, marginTop:1,
                              background: sel ? bloc.color : "#E0E0E0",
                              display:"flex", alignItems:"center", justifyContent:"center",
                            }}>
                              {sel && <span style={{ color:"#fff", fontSize:11, fontWeight:700 }}>✓</span>}
                            </div>
                            <div style={{ flex:1 }}>
                              <div style={{ fontSize:12, fontWeight: sel?600:400, color: sel?bloc.color:"#212121", lineHeight:1.4 }}>
                                {theme.titre}
                              </div>
          ...
          <div style={{ position:"sticky" as const, top:70 }}>
            ...
            {!demandeEnvoyee ? (
              <>
                {/* Titre étape 2 */}
                <div style={{ fontSize:12, fontWeight:700, color:"#424242", marginBottom:12, letterSpacing:".05em" }}>
                  ÉTAPE 2 — Configurez votre formation
                </div>

                <div style={{ background:"#fff", borderRadius:12, padding:20, boxShadow:"0 1px 3px rgba(0,0,0,0.06)", display:"flex", flexDirection:"column" as const, gap:18 }}>
                  ...
                  <div>
                    ...
                    <div className="opt-grid-3">
                      {MODALITES.map(m => (
                        <button key={m.id} className="haki-btn"
                          onClick={() => set("modalite", m.id)}
                  ...
                  {config.modalite !== "enligne" && (
                    <div>
                      ...
                      <div style={{ display:"flex", flexDirection:"column" as const, gap:6 }}>
                        {LIEUX.map(l => (
                          <button key={l.id} className="haki-btn"
                            onClick={() => set("lieu", l.id)}
                            ...
                            {l.emoji} {l.label}
                            <span style={{ fontSize:10, color:"#9E9E9E", marginLeft:6 }}>{l.desc}</span>
                          </button>
                        ))}
                      </div>
                    </div>
                  ...
                  {/* Participants */}
                  <div>
                    <div style={{ fontSize:11, fontWeight:700, color:"#9E9E9E", letterSpacing:".1em", textTransform:"uppercase" as const, marginBottom:8 }}>Nombre de participants</div>
                    <div className="opt-grid-2">
                      {PARTICIPANTS.map(p => (
                        <button key={p.id} className="haki-btn"
                          onClick={() => set("participants", p.id)}
                  ...
                  <div>
                    ...
                    <div style={{ display:"flex", flexDirection:"column" as const, gap:6 }}>
                      {INTERVENANTS.map(i => (
                        <button key={i.id} className="haki-btn"
                          onClick={() => set("intervenants", i.id)}
                          style={{ padding:"9px 12px", textAlign:"left" as const, fontSize:12,
                  ...
                  <div>
                    ...
                    <div className="opt-grid-3">
                      {DUREES.map(d => (
                        <button key={d.id} className="haki-btn"
                          ...
                          <div style={{ fontSize:14, marginBottom:2 }}>{d.emoji}</div>
                          <div>{d.label}</div>
                  ...
                  <div>
                    ...
                    <div style={{ display:"flex", flexDirection:"column" as const, gap:5 }}>
                      {CERTIFICATIONS.map(c => (
                        <button key={c.id} className="haki-btn"
                          onClick={() => set("certification", c.id)}
                  ...
                  <div>
                    <div style={{ fontSize:11, fontWeight:700, color:"#9E9E9E", letterSpacing:".1em", textTransform:"uppercase" as const, marginBottom:8 }}>
                      Contexte complémentaire <span style={{ fontWeight:400, textTransform:"none" as const }}>(optionnel)</span>
                    </div>
                    <textarea value={contexte} onChange={e => setContexte(e.target.value)}
                      placeholder="Ex : Formation pour 25 managers suite à notre diagnostic Haki (score 58/100). Accent particulier souhaité sur le tribalisme dans le recrutement et la gestion des PVVIH..."
                      rows={3}
                      style={{ width:"100%", padding:"10px 12px", border:"1px solid #E0E0E0", borderRadius:8, fontSize:12, fontFamily:"system-ui", resize:"vertical" as const, boxSizing:"border-box" as const }} />
                  </div>

                  {/* Devis estimatif */}
                  <div style={{ background:"linear-gradient(135deg, #1A237E 0%, #283593 100%)", borderRadius:10, padding:"16px 18px" }}>
                    <div style={{ fontSize:10, fontWeight:700, color:"#9FA8DA", letterSpacing:".1em", textTransform:"uppercase" as const, marginBottom:6 }}>
                      Devis estimatif
                    </div>
                    <div style={{ fontSize:28, fontWeight:700, color:"#FFC107", marginBottom:2 }}>
                      {formatFCFA(devis)}
                    </div>
                    <div style={{ fontSize:10, color:"#7986CB", lineHeight:1.5 }}>
                      Estimation indicative · Devis définitif confirmé par Haki sous 48h
                      {nbThemes > 1 && ` · ${nbThemes} thématiques sélectionnées`}
                    </div>
                  </div>
                  ...
                  {/* Bouton soumettre */}
                  <button className="haki-btn" onClick={soumettre}
                    style={{ padding:"14px", background:"#1A237E", color:"#fff", fontSize:14, fontWeight:700 }}>
                    Soumettre ma demande →
                  </button>

                  <div style={{ fontSize:10, color:"#BDBDBD", textAlign:"center" as const, lineHeight:1.5 }}>
                    Aucun engagement avant validation du devis définitif
                  </div>
                </div>
              </>
            ) : (
              ...
              <div style={{ background:"#fff", borderRadius:12, padding:32, boxShadow:"0 1px 3px rgba(0,0,0,0.06)", textAlign:"center" as const }}>
                ...
                <div style={{ fontSize:13, color:"#757575", lineHeight:1.7, marginBottom:16 }}>
                  L'équipe Haki a reçu votre demande de formation pour <strong>{user?.organisationNom}</strong>.
                  Vous recevrez un devis définitif sous <strong>48h</strong> à votre adresse email.
                </div>
                <div style={{ background:"#E8EAF6", borderRadius:8, padding:"12px 16px", marginBottom:20, textAlign:"left" as const }}>
                  <div style={{ fontSize:11, fontWeight:700, color:"#1A237E", marginBottom:6 }}>Récapitulatif de votre demande</div>
                  <div style={{ fontSize:11, color:"#424242", lineHeight:1.8 }}>
                    📋 {nbThemes} thématique{nbThemes>1?"s":""} sélectionnée{nbThemes>1?"s":""}<br/>
                    🏢 {MODALITES.find(m=>m.id===config.modalite)?.label} · {LIEUX.find(l=>l.id===config.lieu)?.label}<br/>
                    ...
                    💰 Devis estimatif : <strong style={{ color:"#1A237E" }}>{formatFCFA(devis)}</strong>
                  </div>
                </div>
                <button className="haki-btn" onClick={() => { setDemandeEnvoyee(false); setThemesSelectionnes([]); setContexte(""); setErreur(""); }}
